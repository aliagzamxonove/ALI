...
from violations import violations_bp, violation_monitor_thread, violations_lock, violations_data
...
# Now start the violation monitoring thread
thread = threading.Thread(target=violation_monitor_thread, daemon=True)
thread.start()
...
USER_CREDENTIALS = {
    'username': 'admin',
    'password': hashlib.sha256('Pass4%33word'.encode()).hexdigest()
}
